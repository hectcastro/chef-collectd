...
remote_file "#{Chef::Config[:file_cache_path]}/collectd-#{node["collectd"]["version"]}.tar.#{node["collectd"]["source_compression_type"]}" do
...
bash "install-collectd" do
  ...
  code <<-EOH
    tar -x#{node["collectd"]["source_compression_type"] == "gz" ? "z" : ""}f collectd-#{node["collectd"]["version"]}.tar.#{node["collectd"]["source_compression_type"]}
